...
from nyc_mobility.evaluation.borough_uncertainty import (
    CONTRASTS,
    family_comparisons,
...
def protocol():
    return tomllib.loads(Path("configs/borough_uncertainty.toml").read_text())


def test_family_intervals_use_three_contrasts_and_correct_percentiles():
    spec = protocol()
    names = ["hist_gradient_boosting", "borough_static", "borough_context"]
    draws = np.array([[10, 8, 12], [10, 9, 11], [10, 10, 10], [10, 11, 9], [10, 12, 8]])
    results = family_comparisons(draws, np.array([10, 10, 10]), names, spec)
    assert len(results) == 3
    assert spec["study"]["family_size"] == 3
    assert {tuple(row[key] for key in ["candidate", "reference"]) for row in results} == {
        (c["candidate"], c["reference"]) for c in CONTRASTS
    }
    for row in results:
        c, r = [names.index(row[key]) for key in ["candidate", "reference"]]
        delta = draws[:, c] - draws[:, r]
        tail = 0.05 / 6
        assert row["delta_mae"] == 0
        assert row["delta_mae_low"] == pytest.approx(np.quantile(delta, 0.025))
        assert row["delta_mae_high"] == pytest.approx(np.quantile(delta, 0.975))
        assert row["delta_mae_family_low"] == pytest.approx(np.quantile(delta, tail))
        assert row["delta_mae_family_high"] == pytest.approx(np.quantile(delta, 1 - tail))
        assert row["per_contrast_confidence_level"] == pytest.approx(1 - 0.05 / 3)
        assert row["family_size"] == 3
        relative = 100 * (1 - draws[:, c] / draws[:, r])
    ...
    # Third contrast shares a candidate but uses a different reference; retain it.
    assert results[2]["delta_mae_family_low"] == pytest.approx(-4 + 8 * 0.05 / 6)
...
def test_protocol_rejects_changed_family_or_invalid_resampling(defect):
    ...
    if defect == "winner_only":
        ...
        spec["contrasts"].append(
            {"candidate": "borough_context", "reference": "previous_week_168h"}
        )
    elif defect == "size":
...
def setup_run(tmp_path):
    ...
    source = tmp_path / "reports/borough/artificial-only"
...
def test_pipeline_reads_only_summaries_and_preserves_prior_reports(tmp_path, monkeypatch):
    ...
    result = run_borough_uncertainty({"split": {"test_start": "2026-03-10"}}, tmp_path, path)
    ...
    saved = tmp_path / "reports/latest_borough_uncertainty.json"
    assert json.loads(saved.read_text())["comparisons"] == result["comparisons"]
    for row in result["comparisons"]:
        expected = {
            ("borough_static", "hist_gradient_boosting"): 1,
            ("borough_context", "hist_gradient_boosting"): -1,
            ("borough_context", "borough_static"): -2,
    ...
    artifact = tmp_path / "artifacts/borough_uncertainty" / result["uncertainty_id"] / "draws.npz"
    ...
    repeated = run_borough_uncertainty({"split": {"test_start": "2026-03-10"}}, tmp_path, path)
    assert repeated["comparisons"] == result["comparisons"]
    assert repeated["uncertainty_id"] != result["uncertainty_id"]
    repeated_artifact = (
        tmp_path / "artifacts/borough_uncertainty" / repeated["uncertainty_id"] / "draws.npz"
    )
    with (
        np.load(artifact, allow_pickle=False) as first,
        np.load(repeated_artifact, allow_pickle=False) as second,
    ):
...
def test_bad_inputs_abort_before_resampling(tmp_path, monkeypatch, defect):
    import nyc_mobility.evaluation.borough_uncertainty as module
    ...
    with pytest.raises(ValueError):
        run_borough_uncertainty({"split": {"test_start": "2026-03-10"}}, tmp_path, path)
    assert not (tmp_path / "reports/borough_uncertainty").exists()
...
@pytest.mark.parametrize("target", ["source", "lock", "input"])
def test_midrun_mutation_aborts_publication(tmp_path, monkeypatch, target):
    import nyc_mobility.evaluation.borough_uncertainty as module
