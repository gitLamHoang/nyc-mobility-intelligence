...
## September 25 progress

Completed the frozen three-contrast borough uncertainty study without fitting: 10,000 paired replicates at each 7/1/14-day block length, actual DST weights, and all nine comparisons. Primary adjusted intervals are [−0.007356, +0.001293] for static minus temporal, [−0.026430, −0.006036] for context minus temporal, and [−0.023611, −0.002951] for context minus static. Static evidence is block-sensitive; both context contrasts retain negative endpoints, but sparse/zero-target harm precludes automatic promotion. Isolated tracked-summary reproduction is exact, 19 previous files are unchanged, 213 tests and Ruff pass, and May remains sealed. [Full review](reports/BOROUGH_UNCERTAINTY_REVIEW.md). Next: authoritative NOAA weather acquisition/quality/availability audit before any weather-model protocol.
...
Completed authoritative [NOAA acquisition and availability preparation](reports/WEATHER_PREPARATION_REVIEW.md) ahead of the planned weather window: three stations, six pinned archives, 14,655 core development observations and 43,476 strictly past snapshot rows across four delay scenarios. The conservative source-223 quality rule fails to cover April after a source transition. This is an explicit policy/metadata gap, not a missing-weather or model-performance claim. No weather ablation was frozen or fitted. Next resolve sources 412/413 quality semantics, re-audit all development months and then freeze a matched comparison. If unresolved, continue interpretation/reproducibility without presenting incomplete weather integration as delivered. All 342 tests, Ruff, actual stage and independent/isolated verification pass; 20 prior files remain unchanged, with May sealed.
